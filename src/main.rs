...
struct PublishedRegistry {
    #[serde(flatten)]
    articles: HashMap<String, PublishedEvent>,
}
...
async fn main() -> Result<()> {
    ...
    match args.command {
        ...
        Commands::Publish { file_name, article_identifier, title, image, summary, published_at } => { action_publish(file_name, article_identifier, title, image, summary, published_at).await? },
...
async fn init_notr_client(args: Args) -> Result<(Keys, Client)> {
...
async fn action_publish(file_name: String, article_identifier: String, title: Option<String>, image: Option<Url>, summary: Option<String>, published_at: Option<u64>) -> Result<()> {
    let args = Args::parse();
    let (keys, client) = init_notr_client(args).await?;
    publish_article(file_name, article_identifier, title, image, summary, published_at, client, keys.public_key()).await
}

async fn action_delete(article_identifier: String) -> Result<()> {
    let args = Args::parse();
    let (keys, client) = init_notr_client(args).await?;
    delete_article(article_identifier, client, keys.public_key()).await
}

async fn action_list(since_published: Option<u64>, until_published: Option<u64>) -> Result<()> {
    let args = Args::parse();
    let (keys, client) = init_notr_client(args).await?;
    list_articles(since_published, until_published, client, keys.public_key()).await
}

async fn init_blog_structure() -> Result<()> {
